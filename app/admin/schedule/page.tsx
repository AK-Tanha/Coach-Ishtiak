            ...
            <div className="lg:col-span-4 p-6 bg-brand-secondary/40 border border-brand-border/80 rounded-[2rem] shadow-lg">
              ...
              <form onSubmit={handleAddClass} className="space-y-4 text-xs">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={newClassTime} onChange={(e) => setNewClassTime(e.target.value)} placeholder="e.g. 5:30 - 7:00 PM" className="w-full bg-brand-primary border border-brand-border p-3 rounded-xl text-white text-sm focus:outline-none focus:border-brand-accent transition-colors" required />
                </div>
                <div className="space-y-1.5">
                  <label className="text-brand-muted font-bold tracking-wider uppercase text-[10px]">Activity / Module</label>
                  <input type="text" value={newClassActivity} onChange={(e) => setNewClassActivity(e.target.value)} placeholder="e.g. MMA / Tactical Muay Thai" className="w-full bg-brand-primary border border-brand-border p-3 rounded-xl text-white text-sm focus:outline-none focus:border-brand-accent transition-colors" required />
                </div>
                          ...
                          <div key={cls.id} className="flex items-center justify-between py-3.5 hover:bg-white/[0.01]">
                            {editingClassDay === dayGroup.day && editingClassId === cls.id ? (
                              <div className="flex-1 flex items-center gap-2">
                                <input type="text" value={editingClassTime} onChange={(e) => setEditingClassTime(e.target.value)} className="flex-1 bg-brand-primary border border-brand-accent p-2 rounded-lg text-white text-xs font-mono focus:outline-none" placeholder="Time" />
                                <input type="text" value={editingClassActivity} onChange={(e) => setEditingClassActivity(e.target.value)} className="flex-[2] bg-brand-primary border border-brand-accent p-2 rounded-lg text-white text-xs focus:outline-none" placeholder="Activity" />
                                <button onClick={saveEditedClass} className="p-2 bg-brand-accent text-black rounded-lg hover:bg-brand-accent-hover transition-colors min-w-[34px] min-h-[34px] flex items-center justify-center cursor-pointer" title="Save">
                                  <Save className="w-3.5 h-3.5" />
                                </button>
                                <button onClick={cancelEditingClass} className="p-2 border border-brand-border text-brand-muted hover:text-white rounded-lg transition-colors min-w-[34px] min-h-[34px] flex items-center justify-center cursor-pointer" title="Cancel">
